...
| 构建工具 | Vite | 5.x |
| UI 框架 | Naive UI | 2.x（Vue 3 原生组件生态） |
| 样式 | Tailwind CSS | 3.x |
| 状态管理 | Pinia | 2.x |
| 路由 | Vue Router | 4.x |
...
### 5.1 设计风格
- **整体**：现代清爽风格，以 Naive UI 默认主题为基础，Tailwind 用于间距、颜色微调。
- **配色**：主色 `#63e2b7`（Naive UI 青绿），辅助色灰色系。
- **图标**：使用 Naive UI 内置图标与 Emoji 组合。
